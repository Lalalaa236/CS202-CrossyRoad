...
void LoadState::drawBoard() {
    ...
    else
    {
        DrawTexturePro(*greyLoadButton,
            ...
            { float(loadButtonX), float(loadButtonY), greyLoadButton->width * scaleWidth, greyLoadButton->height * scaleHeight },
            { 0, 0 },
            0,
            WHITE);
    }
...
void LoadState::handleInput() {
    ...
    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
        ...
        selectedSlot = -1;
        for (int i = 0; i < 3; ++i) {
            if (saveDatas[i].getSerializedData() == "") continue;
